<template>
  ...
  >
    <span style="color: var(--t-color-light-blue)"
      >Showing {{ start }} - {{ end }} of
      {{ dataToDisplay.length }} records</span
    >
    <div>
      <button
        v-if="start >= 2"
      ...
      <button
        v-if="start < 2"
        class="disabled-outline-button"
        style="width: 105px"
      >
        Previous
      </button>
      <button
        v-if="end < dataToDisplay.length"
        @click="getNext"
        class="light-blue-outline-button"
    ...
    data: {
      handler() {
        ...
        if (this.end > this.dataToDisplay.length) {
          this.start =
            this.dataToDisplay.length - (this.show - 1) >= 0
              ? this.dataToDisplay.length - (this.show - 1)
              : 0;
  ...
  methods: {
    ...
    checkEnd() {
      if (this.end > this.dataToDisplay.length) {
        ...
        this.end - this.start < this.show &&
        this.start + this.show <= this.dataToDisplay.length
      ) {
        ...
        this.end - this.start < this.show &&
        this.dataToDisplay.length > this.end
      ) {
