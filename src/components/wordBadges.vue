    ...
    <b-container style="padding-top:5px">
      ...
      <div v-if="badgein.length > 0 & show === true">
        <small>
          ...
          words to add it to your set.<br /><strong>The number beside the keyword
            indicates how frequently the terms appear in the Throughput Database.</strong>
        </small>
      </div>
      <div v-if="show === false">
        <small>
      ...
      <div v-if="badgein.length > 0" class='keyword-container'>
        <div v-for="index in Math.min(30, badgein.length)" :key="index">
          <div v-if="badgein[index-1].show === show" class='keyword-badge' v-bind:class="{'red-badge': !show}">
            <!-- Clicking one of these span elements turns the show parameter
                 on or off.  That happens in "keyword_search"-->
            <span v-on:click="$emit('termOut', badgein[index - 1].term)">{{ badgein[index - 1].term }}</span>
